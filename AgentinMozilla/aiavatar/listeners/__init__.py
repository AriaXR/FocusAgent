...
class SpeechListenerAdvanced:
    ...
    def register_all(self):
        ...
        for participant in self.participants:
            name = participant
            file_path = 'Your_candidate_audio_files/{}.mp3'.format(name)
            self.register(name, file_path)
        ...
        while True:
            ...
            if not transcript:
                ...
                if self.empty_ASR >= 5:
                    ...
                    self.is_listening = False
                continue
            transcript = whisperx.align(transcript, self.aligner, self.metadata, item, self.device, return_char_alignments=False)['segments']
            text = ""
            for i in range(len(transcript)):
                ...
                if item[start_time:end_time].shape[0] > 1024:
                    audio_embedding = self.voice_encoder.encode_batch(torch.from_numpy(item[start_time:end_time][numpy.newaxis, :]))[0]
